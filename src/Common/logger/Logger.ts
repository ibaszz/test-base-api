...
const LOG_FORMAT = format.printf((info) => `${info.timestamp} ${info.level}: ${info.message} ${JSON.stringify(info.context)}`);
...
export class Logger implements LoggerService {
    ...
    constructor(moduleName?: string) {
        ...
        new transports.Console({
            ...
            format: NON_DEV_ENV ? format.combine(format.timestamp(), METADATA_FORMAT, format.json())
            : format.combine(format.timestamp(), format.colorize(), METADATA_FORMAT, LOG_FORMAT)}),
        );
    ...
    private createLog(level, message: any, context?: object, category?: string) {
        const actualMessage = typeof message['message'] !== 'undefined' ? message['message'] : message;
        const stringMessage =
        typeof actualMessage === 'object' ? JSON.stringify(actualMessage, null, 2) : actualMessage;
        this.logger.log({
            level,
            message: this.moduleName ? `[${this.moduleName}] ${stringMessage}` : stringMessage,
            ...context,
            category,
